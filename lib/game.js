...
class Game {
  constructor(canvas, far, mid, near, fore, house, buffer) {
    this.canvas = canvas;
    this.farCanvas = far;
    this.midCanvas = mid;
  ...
  initializeGame() {
    ...
    this.buffer.addEventListener('click', (e) => this.touchJump(e));
  }
  ...
  touchJump(e) {
    if (e.target.id === 'canvas-buffer' && this.littleRed.jumps < 3) {
      this.littleRed.jump();
    }
  }
