...
class Game {
  ...
  addLevel() {
    this.level += 1;
    this.bigBadWolf.minXPos += 150;
    this.foreGround.text = new ScreenText(this.context, this.level);
    if (this.sound) this.bigBadWolf.growl();
  }

  checkLevel() {
    if (this.points >= 50 && this.level === 1) {
      this.addLevel();
    } else if (this.points >= 100 && this.level === 2) {
      this.addLevel();
    } else if (this.points >= 150 && this.level === 3) {
      this.addLevel();
    }
  }
